...
async def _get_all_chunks(url, session: tidalapi.Session, parser, obs=None, params={}) -> List[tidalapi.Track]:
    """
    ...
    """
    if obs is None:
        obs = "Fetching additional data chunks"
...
async def get_all_playlists(user: tidalapi.User, chunk_size: int=10) -> List[tidalapi.Playlist]:
    ...
    desc = "> Loading Tidal playlists"
    return await _get_all_chunks(f"users/{user.session.user.id}/playlists", session=user.session, parser=user.playlist.parse_factory, obs=desc, params=params)
...
async def get_all_playlist_tracks(playlist: tidalapi.Playlist, chunk_size: int=20) -> List[tidalapi.Track]:
    ...
    desc = f" > Loading Tidal playlist '{playlist.name}'"
    return await _get_all_chunks(f"{playlist._base_url%playlist.id}/tracks", session=playlist.session, parser=playlist.session.parse_track, obs=desc, params=params)
###
