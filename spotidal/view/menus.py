...
class Prompt(Enum):
    ...
    kb_answer = {"answer": [{"key": "enter"}, {"key": "space"}]}
...
class PromptFactory:
    ...
    def create(
        ...
        if prompt_type == Prompt.SEARCH:
            prompt["max_height"] = "70%"
...
class MainMenu(MenuBase):
    def __init__(self):
        ...
        self.options = [opt[0] for opt in self.MAIN_OPT]
...
class SelectionModeMenu(MenuBase):
    ...
    def __init__(self):
        super().__init__(Prompt.LIST)
        self.options = [opt[0] for opt in self.SELECTION_OPT]
